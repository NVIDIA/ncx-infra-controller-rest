...
type APIDpuExtensionServiceVersionInfo struct {
	...
	Observability *APIDpuExtensionServiceObservability `json:"observability,omitempty"`
}
...
func (desoc APIDpuExtensionServiceObservabilityConfig) Validate() error {
	if desoc.Name != nil && len(*desoc.Name) > DpuExtensionServiceMaxObservabilityConfigNameLength {
...
// Accepts an observability definition from an API request and converts
// it to the proto representation that will be stored and passed
// to Carbide
func ProtobufObservabilityFromAPIDpuExtensionServiceObservability(apiObservability *APIDpuExtensionServiceObservability) *cdbm.DpuExtensionServiceObservability {
	...
	return &cdbm.DpuExtensionServiceObservability{
		DpuExtensionServiceObservability: protoObservability,
	}
}

// Accepts a Carbide version definition and converts
// it to the db representation that will be
// stored in cloud
func DBDpuExtensionServiceVersionInfoFromProto(versionInfo *cwssaws.DpuExtensionServiceVersionInfo, fallbackTime time.Time) *cdbm.DpuExtensionServiceVersionInfo {
	if versionInfo == nil {
		return nil
	}

	created, err := time.Parse(DpuExtensionServiceTimeFormat, versionInfo.Created)
	if err != nil {
	...
	if versionInfo.Observability != nil {
		observability = &cdbm.DpuExtensionServiceObservability{
			DpuExtensionServiceObservability: versionInfo.Observability,
		}
	}

	return &cdbm.DpuExtensionServiceVersionInfo{
...
// Accepts a Carbide version definition and converts
// it to the carbide-rest-api representation that will be
// returned to users
func APIDpuExtensionServiceVersionInfoFromProto(versionInfo *cwssaws.DpuExtensionServiceVersionInfo, fallbackTime time.Time) *APIDpuExtensionServiceVersionInfo {
	dbVersionInfo := DBDpuExtensionServiceVersionInfoFromProto(versionInfo, fallbackTime)
	if dbVersionInfo == nil {
		return nil
	}

	return NewAPIDpuExtensionServiceVersionInfo(dbVersionInfo)
}

// Accepts a Carbide observability definition and converts
// it to the carbide-rest-api representation that will be
// returned to users
func apiObservabilityFromProto(protoObservability *cwssaws.DpuExtensionServiceObservability) *APIDpuExtensionServiceObservability {
	if protoObservability == nil {
		return nil
	}

	apiObservability := &APIDpuExtensionServiceObservability{
		Configs: make([]APIDpuExtensionServiceObservabilityConfig, 0, len(protoObservability.Configs)),
	}
...
func NewAPIDpuExtensionServiceObservability(dbo *cdbm.DpuExtensionServiceObservability) *APIDpuExtensionServiceObservability {
	...
	return apiObservabilityFromProto(dbo.DpuExtensionServiceObservability)
}
