	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				reqUser:     tnu1,
				respCode:    http.StatusBadRequest,
				respMessage: "The primary physical interface must use a VPC prefix that matches with Instance VPC",
			},
			wantErr: false,
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				reqUser:            tnu1,
				respCode:           http.StatusBadRequest,
				respMessage:        cdb.GetStrPtr("The primary physical interface must use a VPC prefix that matches with Instance VPC"),
				respNoOfInterfaces: cdb.GetIntPtr(1),
			},
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.args.expectedSecondaryVpcIDs != nil {
				assert.ElementsMatch(t, tt.args.expectedSecondaryVpcIDs, rst.SecondaryVpcIDs)
			}
