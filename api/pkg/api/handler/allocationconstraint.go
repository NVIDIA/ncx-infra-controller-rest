	...
	if ac.ConstraintValue != apiRequest.ConstraintValue {
		...
		case cdbm.AllocationResourceTypeInstanceType:
			...
			if serr != nil {
				...
				return cutil.NewAPIErrorResponse(c, http.StatusInternalServerError, "Failed to retrieve Instances for Allocation Constraint, DB error", nil)
			}
			if iCount > apiRequest.ConstraintValue {
				...
				return cutil.NewAPIErrorResponse(c, http.StatusBadRequest, "Cannot update Allocation Constraint, number of active Instances for Allocation is greater than the requested constraint value", nil)
			}
		...
		// Validate if Tenant has Subnet or VPC Prefix using this IP Block
		case cdbm.AllocationResourceTypeIPBlock:
			...
			// Check if Tenant has Subnets using this IP Block
			...
			// Check if Tenant has VPC Prefixes using this IP Block
