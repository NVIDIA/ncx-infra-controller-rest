	...
	if ac.ConstraintValue != apiRequest.ConstraintValue {
		...
		case cdbm.AllocationResourceTypeInstanceType:
			...
			allocConstraints, _, serr := acDAO.GetAll(
				ctx, tx, allocationIDs, nil, []uuid.UUID{dbit.ID}, nil, nil, nil, nil, cdb.GetIntPtr(paginator.TotalLimit), nil,
			)
			if serr != nil {
			...
			_, iCount, serr := inDAO.GetAll(ctx, tx, cdbm.InstanceFilterInput{
