...
func (cah CreateAllocationHandler) Handle(c echo.Context) error {
	...
	err = cdb.WithTx(ctx, cah.dbSession, func(tx *cdb.Tx) error {
		...
		if derr := tx.TryAcquireAdvisoryLock(ctx, cdb.GetAdvisoryLockIDFromString(lockID), nil); derr != nil {
		...
		if _, _, derr := common.GetAllAllocationConstraintsForInstanceType(ctx, tx, cah.dbSession, ip, site, tenant, nil); derr != nil {
		...
		for _, ac := range apiRequest.AllocationConstraints {
			...
			case cdbm.AllocationResourceTypeInstanceType:
				...
				if ac.ConstraintType == cdbm.AllocationConstraintTypeReserved {
					...
					if derr := tx.TryAcquireAdvisoryLock(ctx, cdb.GetAdvisoryLockIDFromString(it.ID.String()), nil); derr != nil {
...
func (dah DeleteAllocationHandler) Handle(c echo.Context) error {
	...
	err = cdb.WithTx(ctx, dah.dbSession, func(tx *cdb.Tx) error {
		...
		if _, _, derr := common.GetAllAllocationConstraintsForInstanceType(ctx, tx, dah.dbSession, ip, a.Site, a.Tenant, nil); derr != nil {
			logger.Error().Err(derr).Msg("error getting count of Allocation Constraints for Instance Type")
		...
		for _, ac := range acs {
			...
			if derr := acDAO.DeleteByID(ctx, tx, ac.ID); derr != nil {
		...
		if derr := aDAO.Delete(ctx, tx, a.ID); derr != nil {
		...
		if acount == 0 {
			...
			if tscount > 0 {
				if derr := tsDAO.Delete(ctx, tx, tss[0].ID); derr != nil {
