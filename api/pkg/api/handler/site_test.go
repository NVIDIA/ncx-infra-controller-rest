...
func TestUpdateSiteHandler_Handle(t *testing.T) {
	...
	mOrg := "test-mixed-org"
	mixedRole := []string{"NICO_PROVIDER_ADMIN", "NICO_TENANT_ADMIN"}
...
func TestGetSiteHandler_Handle(t *testing.T) {
	...
	ipRoles2 := []string{"NICO_PROVIDER_ADMIN"}
...
//	[]
//
//	~ nicocli site create --name "reno-qa4"
...
func TestGetAllSiteHandler_NullConfig(t *testing.T) {
	...
	t.Logf("--- nicocli site list ---")
	t.Logf("%d items", pr.Total)
	t.Logf("%s", listRec.Body.String())

	// ---- Step 2: nicocli site create --name "reno-qa4" ----
	...
	t.Logf("--- nicocli site create --name reno-qa4 ---")
