...
func (csh CreateOperatingSystemHandler) Handle(c echo.Context) error {
	...
	actingAsProvider := isProviderAdmin
	...
	if actingAsProvider && apiRequest.IpxeTemplateName == nil {
	...
	if apiRequest.IpxeScript != nil || apiRequest.IpxeTemplateName != nil {
	...
	//   - image-based OS (tenant):    siteIds required, validated against TenantSite
	//   - image-based OS (provider):  siteIds required, validated against provider's sites
	//   - iPXE + scope == "global":   siteIds forbidden; auto-associate ALL registered provider sites
	//   - iPXE + scope == "limited":  siteIds required, validated against provider sites
	//   - iPXE + no scope:            no site associations created (legacy behaviour)
	rdbst := []cdbm.Site{}
	dbossd := []cdbm.StatusDetail{}
	sttsmap := map[uuid.UUID]*cdbm.TenantSite{} // populated only for tenant-owned image-based OS

	isGlobal := osType == cdbm.OperatingSystemTypeIPXE && apiRequest.Scope != nil && *apiRequest.Scope == model.OsScopeGlobal
	isLimited := osType == cdbm.OperatingSystemTypeIPXE && apiRequest.Scope != nil && *apiRequest.Scope == model.OsScopeLimited
...
func (ush UpdateOperatingSystemHandler) Handle(c echo.Context) error {
	...
	} else if uos.Type == cdbm.OperatingSystemTypeIPXE {
		for _, dbossa := range dbossas {
			stc, err := ush.scp.GetClientByID(dbossa.SiteID)
			if err != nil {
				logger.Error().Err(err).Msg("failed to retrieve Temporal client for Site")
			...
			updateOsRequest := &cwssaws.UpdateOperatingSystemRequest{
				...
				IpxeParameters:     dbParamsToProto(uos.IpxeParameters),
				IpxeArtifacts:      dbArtifactsToProto(uos.IpxeArtifacts),
