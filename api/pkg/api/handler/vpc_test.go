	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				reqUser:     tnu,
				respCode:    http.StatusBadRequest,
				respMessage: "`routingProfileType` can only be specified if network virtualization type is set to `FNN`, or Site has native networking enabled and no network virtualization type is specified",
			},
			wantErr:            false,
			verifyChildSpanner: true,
		},
