...
build:
	...
	cd cli && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "-extldflags '-static'" -o ../$(BUILD_DIR)/nicocli ./cmd/nicocli

INSTALL_DIR ?= $(shell go env GOPATH)/bin

nico-cli:
	go build -o $(INSTALL_DIR)/nicocli ./cli/cmd/nicocli
