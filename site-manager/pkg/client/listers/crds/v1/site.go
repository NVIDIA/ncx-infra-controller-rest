...
type SiteLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Site, err error)
...
// List lists all Sites in the indexer.
func (s *siteLister) List(selector labels.Selector) (ret []*v1.Site, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Site))
	})
	return ret, err
}
...
// List lists all Sites in the indexer for a given namespace.
func (s siteNamespaceLister) List(selector labels.Selector) (ret []*v1.Site, err error) {
...
// Get retrieves the Site from the indexer for a given namespace and name.
func (s siteNamespaceLister) Get(name string) (*v1.Site, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("site"), name)
	}
	return obj.(*v1.Site), nil
}
