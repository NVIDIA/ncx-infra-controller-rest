...
// TestNicoVpc - test the nico grpc client
func TestNicoVpc(t *testing.T) {
	TestInitElektra(t)
	grpcClient := testElektra.manager.API.Nico.GetGRPCClient()
	...
	for _, tc := range tcs {
		t.Run(tc.descr, func(t *testing.T) {
			...
			case "create":
				...
				ctx, span := otel.Tracer(os.Getenv("LS_SERVICE_NAME")).Start(ctx, "NicoTest-CreateVPC")
				...
				span.End()
				nico.ManagerAccess.API.Nico.UpdateGRPCClientState(err)
				if err != nil {
					t.Log(err.Error())
				}
				assert.Nil(t, err)
				...
				rpcSucc++
				assert.Equal(t, 0,
					int(nico.ManagerAccess.Data.EB.Managers.Nico.State.GrpcFail.Load()))
				assert.Equal(t, rpcSucc,
					int(nico.ManagerAccess.Data.EB.Managers.Nico.State.GrpcSucc.Load()))
				vpc = response
				t.Log("GRPCResponse", response)
			case "get":
				ctx := context.Background()
				ctx, span := otel.Tracer(os.Getenv("LS_SERVICE_NAME")).Start(ctx, "NicoTest-VpcSearchQuery")
				resq, err := grpcClient.Networks().GetAllVPCs(ctx, &wflows.VpcSearchFilter{}, utils.NicoApiPageSize)
				span.End()
				nico.ManagerAccess.API.Nico.UpdateGRPCClientState(err)
				if err != nil {
					t.Log(err.Error())
			...
			case "delete":
				ctx := context.Background()
				ctx, span := otel.Tracer(os.Getenv("LS_SERVICE_NAME")).Start(ctx, "NicoTest-DeleteVPC")
...
// TestNicoSubnet - test the nico grpc client
func TestNicoSubnet(t *testing.T) {
	TestInitElektra(t)
	grpcClient := testElektra.manager.API.Nico.GetGRPCClient()
	...
	for _, tc := range tcs {
		t.Run(tc.descr, func(t *testing.T) {
			...
			case "create":
				...
				ctx := context.Background()
				ctx, span := otel.Tracer(os.Getenv("LS_SERVICE_NAME")).Start(ctx, "NicoTest-CreateNetworkSegment")
			...
			case "get":
				ctx := context.Background()
				ctx, span := otel.Tracer(os.Getenv("LS_SERVICE_NAME")).Start(ctx, "NicoTest-GetNetworkSegment")
			...
			case "delete":
				...
				ctx := context.Background()
				ctx, span := otel.Tracer(os.Getenv("LS_SERVICE_NAME")).Start(ctx, "NicoTest-DeleteNetworkSegment")
