...
func NewElektraConfig(utMode bool) *conftypes.Config {
	...
	// Nico config
	flag.StringVar(&conf.Nico.Address, "nicoAddress", os.Getenv("NICO_ADDRESS"), "Nico Address")
	if conf.Nico.Address == "" {
		conf.Nico.Address = "nico-api.nico-system.svc.cluster.local:1079"
	}
	cSecOpt, err := strconv.Atoi(os.Getenv("NICO_SEC_OPT"))
	if err != nil {
	...
	flag.StringVar(&conf.Nico.ServerCAPath, "nicoCertPath", os.Getenv("NICO_CA_CERT_PATH"), "Nico Cert Path")
	if conf.Nico.ServerCAPath == "" {
		conf.Nico.ServerCAPath = DefaultNicoClientCAPath
	}
	flag.StringVar(&conf.Nico.ClientCertPath, "nicoClientCertPath", os.Getenv("NICO_CLIENT_CERT_PATH"), "Nico client Cert Path")
	if conf.Nico.ClientCertPath == "" {
		conf.Nico.ClientCertPath = DefaultNicoClientCertPath
	}
	flag.StringVar(&conf.Nico.ClientKeyPath, "nicoClientKeyPath", os.Getenv("NICO_CLIENT_KEY_PATH"), "Nico client Cert Path")
