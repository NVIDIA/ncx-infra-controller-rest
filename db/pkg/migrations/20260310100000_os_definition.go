...
func init() {
	Migrations.MustRegister(func(ctx context.Context, db *bun.DB) error {
		...
		_, err = tx.Exec("ALTER TABLE operating_system ADD COLUMN IF NOT EXISTS ipxe_artifacts JSONB NULL")
		handleError(tx, err)

		_, err = tx.Exec("ALTER TABLE operating_system ADD COLUMN IF NOT EXISTS definition_hash TEXT NULL")
		handleError(tx, err)
