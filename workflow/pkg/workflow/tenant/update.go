...
func UpdateTenantInventory(ctx workflow.Context, siteID string, tenantInventory *cwssaws.TenantInventory) (err error) {
	...
	err = workflow.ExecuteActivity(ctx, tenantManager.UpdateTenantsInDB, parsedSiteID, tenantInventory).Get(ctx, nil)
	if err != nil {
		logger.Warn().Err(err).Msg("failed to execute activity: UpdateTenantsInDB")
	}

	// Record latency for this inventory call
	var inventoryMetricsManager cwm.ManageInventoryMetrics

	serr := workflow.ExecuteActivity(ctx, inventoryMetricsManager.RecordLatency, parsedSiteID, "UpdateTenantInventory", err != nil, time.Since(startTime)).Get(ctx, nil)
	if serr != nil {
