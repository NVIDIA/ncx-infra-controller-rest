...
func (mit ManageIpxeTemplate) UpdateIpxeTemplatesInDB(ctx context.Context, siteID uuid.UUID, inventory *cwssaws.IpxeTemplateInventory) error {
	...
	// Build a map of existing templates keyed by name (natural key from core)
	existingByName := map[string]*cdbm.IpxeTemplate{}
	for i := range existingTemplates {
		existingByName[existingTemplates[i].Name] = &existingTemplates[i]
	}

	// Track all names reported by this inventory payload (either from full list in pagination or iteration on current load)
	...
	if inventory.InventoryPage != nil {
		...
		for _, name := range inventory.InventoryPage.ItemIds {
	...
	for _, reported := range inventory.GetTemplates() {
		...
		if reported.Name == "" {
			logger.Error().Msg("Received iPXE template with empty name, skipping")
			continue
		}

		reportedNames[reported.Name] = true
		reportedScope := ipxeScopeToString(reported.Scope)

		cur, found := existingByName[reported.Name]
		if !found {
			// Create new template
	...
	// When paging is enabled, reportedNames is seeded from InventoryPage.ItemIds (all pages)
	// so we only reconcile on the last page once the full set of names is known.
	if inventory.InventoryPage == nil || inventory.InventoryPage.TotalPages == 0 ||
		inventory.InventoryPage.CurrentPage == inventory.InventoryPage.TotalPages {
		for _, existing := range existingTemplates {
			if reportedNames[existing.Name] {
				continue
			}
