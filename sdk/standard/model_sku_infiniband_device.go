/*
NCX Infra Controller REST API

NCX Infra Controller REST API allows users to create and manage resources e.g. VPC, Subnets, Instances across all connected NCX Infra Controller datacenters, also referred to as Sites.

API version: 1.4.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package standard

import (
	"encoding/json"
)

// checks if the SkuInfinibandDevice type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SkuInfinibandDevice{}

// SkuInfinibandDevice Infiniband device component in a SKU
type SkuInfinibandDevice struct {
	// Vendor of the infiniband device
	Vendor *string `json:"vendor,omitempty"`
	// Model of the infiniband device
	Model *string `json:"model,omitempty"`
	// Number of infiniband devices present
	Count *int32 `json:"count,omitempty"`
}

// NewSkuInfinibandDevice instantiates a new SkuInfinibandDevice object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSkuInfinibandDevice() *SkuInfinibandDevice {
	this := SkuInfinibandDevice{}
	return &this
}

// NewSkuInfinibandDeviceWithDefaults instantiates a new SkuInfinibandDevice object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSkuInfinibandDeviceWithDefaults() *SkuInfinibandDevice {
	this := SkuInfinibandDevice{}
	return &this
}

// GetVendor returns the Vendor field value if set, zero value otherwise.
func (o *SkuInfinibandDevice) GetVendor() string {
	if o == nil || IsNil(o.Vendor) {
		var ret string
		return ret
	}
	return *o.Vendor
}

// GetVendorOk returns a tuple with the Vendor field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SkuInfinibandDevice) GetVendorOk() (*string, bool) {
	if o == nil || IsNil(o.Vendor) {
		return nil, false
	}
	return o.Vendor, true
}

// HasVendor returns a boolean if a field has been set.
func (o *SkuInfinibandDevice) HasVendor() bool {
	if o != nil && !IsNil(o.Vendor) {
		return true
	}

	return false
}

// SetVendor gets a reference to the given string and assigns it to the Vendor field.
func (o *SkuInfinibandDevice) SetVendor(v string) {
	o.Vendor = &v
}

// GetModel returns the Model field value if set, zero value otherwise.
func (o *SkuInfinibandDevice) GetModel() string {
	if o == nil || IsNil(o.Model) {
		var ret string
		return ret
	}
	return *o.Model
}

// GetModelOk returns a tuple with the Model field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SkuInfinibandDevice) GetModelOk() (*string, bool) {
	if o == nil || IsNil(o.Model) {
		return nil, false
	}
	return o.Model, true
}

// HasModel returns a boolean if a field has been set.
func (o *SkuInfinibandDevice) HasModel() bool {
	if o != nil && !IsNil(o.Model) {
		return true
	}

	return false
}

// SetModel gets a reference to the given string and assigns it to the Model field.
func (o *SkuInfinibandDevice) SetModel(v string) {
	o.Model = &v
}

// GetCount returns the Count field value if set, zero value otherwise.
func (o *SkuInfinibandDevice) GetCount() int32 {
	if o == nil || IsNil(o.Count) {
		var ret int32
		return ret
	}
	return *o.Count
}

// GetCountOk returns a tuple with the Count field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SkuInfinibandDevice) GetCountOk() (*int32, bool) {
	if o == nil || IsNil(o.Count) {
		return nil, false
	}
	return o.Count, true
}

// HasCount returns a boolean if a field has been set.
func (o *SkuInfinibandDevice) HasCount() bool {
	if o != nil && !IsNil(o.Count) {
		return true
	}

	return false
}

// SetCount gets a reference to the given int32 and assigns it to the Count field.
func (o *SkuInfinibandDevice) SetCount(v int32) {
	o.Count = &v
}

func (o SkuInfinibandDevice) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SkuInfinibandDevice) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Vendor) {
		toSerialize["vendor"] = o.Vendor
	}
	if !IsNil(o.Model) {
		toSerialize["model"] = o.Model
	}
	if !IsNil(o.Count) {
		toSerialize["count"] = o.Count
	}
	return toSerialize, nil
}

type NullableSkuInfinibandDevice struct {
	value *SkuInfinibandDevice
	isSet bool
}

func (v NullableSkuInfinibandDevice) Get() *SkuInfinibandDevice {
	return v.value
}

func (v *NullableSkuInfinibandDevice) Set(val *SkuInfinibandDevice) {
	v.value = val
	v.isSet = true
}

func (v NullableSkuInfinibandDevice) IsSet() bool {
	return v.isSet
}

func (v *NullableSkuInfinibandDevice) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSkuInfinibandDevice(val *SkuInfinibandDevice) *NullableSkuInfinibandDevice {
	return &NullableSkuInfinibandDevice{value: val, isSet: true}
}

func (v NullableSkuInfinibandDevice) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSkuInfinibandDevice) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
