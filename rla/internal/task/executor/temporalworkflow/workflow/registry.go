...
// unmarshalAndValidate returns an Unmarshal function that decodes raw JSON
// into a new *T and calls Validate() on the result. The pointer is returned
// as any so Temporal's codec can deserialize it into the workflow's typed
// parameter at dispatch time.
