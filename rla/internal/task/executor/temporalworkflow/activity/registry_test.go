...
// state: the updater and registry fields are independent.
func TestActivities_Isolation(t *testing.T) {
	a1 := New(nil, nil)
	a2 := New(nil, nil)

	// Both instances should return separate map objects.
	m1 := a1.All()
	m2 := a2.All()
	assert.NotSame(t, &m1, &m2, "All() must return distinct map instances")
}
